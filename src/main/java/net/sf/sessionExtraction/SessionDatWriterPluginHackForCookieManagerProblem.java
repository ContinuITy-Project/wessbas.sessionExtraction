...
@Plugin(description = "A filter to print the object to a configured stream", configuration = {
		@Property(name = SessionDatWriterPluginHackForCookieManagerProblem.CONFIG_PROPERTY_NAME_STREAM, defaultValue = SessionDatWriterPluginHackForCookieManagerProblem.CONFIG_PROPERTY_VALUE_STREAM_STDOUT, description = "The name of the stream used to print the incoming data (special values are STDOUT, STDERR, STDlog, and NULL; "
				+ "other values are interpreted as filenames)."),
		@Property(name = SessionDatWriterPluginHackForCookieManagerProblem.CONFIG_PROPERTY_NAME_ENCODING, defaultValue = SessionDatWriterPluginHackForCookieManagerProblem.CONFIG_PROPERTY_VALUE_DEFAULT_ENCODING, description = "The used encoding for the selected stream."),
		@Property(name = SessionDatWriterPluginHackForCookieManagerProblem.CONFIG_PROPERTY_NAME_APPEND, defaultValue = SessionDatWriterPluginHackForCookieManagerProblem.CONFIG_PROPERTY_VALUE_STREAM_APPEND, description = "Decides whether the filter appends to the stream in the case of a file or not.") })
public final class SessionDatWriterPluginHackForCookieManagerProblem extends AbstractFilterPlugin {
	 ...
	 */
	public SessionDatWriterPluginHackForCookieManagerProblem(final Configuration configuration,
	...
	public final void inputEvent(final ExecutionTraceBasedSession session) {
		if (this.active) {
			...
			int curSubSessionId = 0;
			String curSessionId = session.getSessionId()+"-"+curSubSessionId; 
